...
def load_dim(df_dim: pd.DataFrame, table: str, unique_col: str, engine) -> pd.DataFrame:
    ...
    df_dim.to_sql(table, engine, if_exists='append', index=False,
...
try:
    df_usuarios.to_sql('dim_usuario', engine, if_exists='append', index=False,
                       method='multi')
...
try:
    df_artistas.to_sql('dim_artista', engine, if_exists='append', index=False,
                       method='multi')
except Exception:
    pass
...
try:
    df_albums.to_sql('dim_album', engine, if_exists='append', index=False,
                     method='multi')
except Exception:
    pass
...
df_faixas = (df[['track_name', 'title', 'artist_name', 'album_name']]
             ...
             .merge(df_art_ids, on='artist_name')
             .merge(df_alb_ids[['id_album', 'album_name', 'id_artista']],
                    on=['album_name', 'id_artista'], how='left')
             .drop(columns=['artist_name', 'album_name']))
try:
    df_faixas.to_sql('dim_faixa', engine, if_exists='append', index=False,
                     method='multi')
except Exception:
    pass
...
df_tempo = (df[['timestamp_unix', 'datetime_utc', 'year', 'month', 'month_name',
            ...
            .rename(columns={
                ...
                'year': 'ano', 'month': 'mes', 'month_name': 'nome_mes',
                'day': 'dia', 'hour': 'hora', 'weekday': 'dia_semana',
                'weekday_name': 'nome_dia', 'week_of_year': 'semana_ano',
...
df_fact.to_sql('fact_scrobbles', engine, if_exists='append', index=False,
...
QUERIES = {
    '1. Ranking geral de artistas do grupo': '''
        ...
        FROM   fact_scrobbles f
        JOIN   dim_artista a ON f.id_artista = a.id_artista
        GROUP  BY artista
    ...
    '3. Artista favorito por usuário': '''
        SELECT username, artista_favorito, scrobbles
        FROM (
            SELECT u.username,
    ...
    '5. Top 10 faixas do grupo': '''
        SELECT fx.track_name  AS faixa,
               a.artist_name  AS artista,
               SUM(f.scrobbles) AS total_scrobbles
        FROM   fact_scrobbles f
        JOIN   dim_faixa   fx ON f.id_faixa   = fx.id_faixa
        JOIN   dim_artista a  ON f.id_artista = a.id_artista
