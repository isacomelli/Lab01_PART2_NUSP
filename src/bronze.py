...
USERNAMES = {
    ...
    ,'Joao':    'jopefroes'
    ,'Pivas':    'e_o_pivas'
    ,'Lucas (Trab)':    'lucasntss'
    ,'Dora (Joao)': 'doraaraujo_'
    ,'Sarah (Joao)': 'aquariaty'
    ,'Brasao (Sarinha)': 'Brasaum'
    ,'Lucas (Nico)': 'lucasraikou'
    ,'Davi (Nico)': 'davizooca'
}
...
def collect_user_tracks(display_name: str, username: str, checkpoint: dict, from_timestamp: int = None) -> list[dict]:
    ...
    # usa o from_timestamp passado, ou o global
...
def main():
    ...
    for display_name, username in USERNAMES.items():
        ...
        if (checkpoint.get(username, {}).get("last_page") ==
            checkpoint.get(username, {}).get("total_pages") and
            checkpoint.get(username, {}).get("total_pages") is not None):
